  ...
  return (
    <FlexWrapper direction="row" gap={5} align="center">
      {["A", "B", "C", "D", "F"].map((score, index) => {
        ...
        return (
          <Grade key={index} selected={isSelected} onClick={() =>
            setGrade(isSelected ? 0 : 5 - index)
          }>
            <FlexWrapper direction="row" justify="center" align="center" style={{ width: "100%", height: "100%" }} gap={0}>
              <Typography type="NormalBold" color="Text.bright">{score}</Typography>
            </FlexWrapper>
          </Grade>
        );
      })}
